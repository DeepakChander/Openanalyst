...
**Tags:** multi-agent, communication, message-passing, events, coordination
...
### [HIGH] Message schemas change but not all agents update
...
### [MEDIUM] Agent assumes messages arrive in order they were sent
...
### [HIGH] Messages lost in transit cause silent failures
...
### [CRITICAL] Agents react to events by emitting more events, causing cascade
...
|---------|-------------|--------|
| `orchestration|coordination|workflow` | multi-agent-orchestration | Need higher-level orchestration patterns |
| `testing|benchmark` | agent-evaluation | Need to test communication patterns |
| `single agent|autonomous` | autonomous-agents | Need single-agent implementation |

### Receives Work From

- **multi-agent-orchestration**: Need inter-agent communication patterns
- **autonomous-agents**: Single agent needs to communicate
...
Full skill path: `~/.spawner/skills/ai-agents/agent-communication/`
