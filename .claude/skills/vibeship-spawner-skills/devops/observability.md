...
**Tags:** observability, logging, metrics, tracing, monitoring, sentry, prometheus, opentelemetry
...
### [CRITICAL] Logs without correlation IDs

**Situation:** Production error. You grep logs. 50,000 lines. No way to connect
...
**Situation:** Security audit. They check logs. Full credit card numbers, passwords,
...
### [HIGH] Metrics labels with too many values

**Situation:** You add user_id as a metric label. 1 million users. Now you have
...
### [HIGH] Too many noisy alerts

**Situation:** On-call gets 200 alerts per day. Most are noise. Real incidents
...
### [MEDIUM] Traces don't connect across services

**Situation:** Distributed system. Request goes through 5 services. Trace shows
...
### [MEDIUM] Wrong log levels in production

**Situation:** Debug logging enabled in production. Logs are 10GB/day. Storage
...
|---------|-------------|--------|
| `user needs performance profiling` | performance-optimization | Performance investigation |
| `user needs kubernetes monitoring` | kubernetes | K8s-specific observability |
| `user needs database slow query logging` | postgres-wizard | Database monitoring |
| `user needs CI/CD pipeline monitoring` | cicd-pipelines | Pipeline observability |
...
Full skill path: `~/.spawner/skills/devops/observability/`
