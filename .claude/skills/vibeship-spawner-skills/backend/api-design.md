...
**Tags:** api, rest, http, versioning, pagination, openapi, swagger
...
### [CRITICAL] Breaking changes without versioning

**Situation:** You rename a field from "userName" to "username". Deploy. Every client
...
### [HIGH] API design causing N+1 requests

**Situation:** Client fetches /users, gets 50 users. Each user has orders. Client
...
### [HIGH] Non-idempotent operations causing duplicates

**Situation:** User clicks "Pay" button. Request times out. They click again.
...
### [MEDIUM] Unhelpful error messages

**Situation:** API returns { "error": "Bad Request" }. What's bad? Which field?
...
**Situation:** GET /users returns array. GET /users/:id returns object with "data"
...
### [MEDIUM] Exposing internal implementation details

**Situation:** Error returns Prisma error with query details. IDs are sequential
...
|---------|-------------|--------|
| `user needs GraphQL` | graphql-schema | GraphQL API design |
| `user needs authentication` | authentication-oauth | API authentication |
| `user needs caching` | redis-specialist | API response caching |
| `user needs database design` | postgres-wizard | Data model for API |
...
Full skill path: `~/.spawner/skills/backend/api-design/`
